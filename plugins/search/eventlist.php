<?php
/**
 * @version 0.9 $Id: eventlist.php 825 2008-12-03 16:12:24Z julienv $
 * @package Joomla
	...
	if(in_array('elevents', $areas)) {
		...
		switch ($phrase) {
			case 'exact':
				...
				$wheres2 	= array();
				$wheres2[] 	= 'LOWER(a.title) LIKE '.$search;
				$wheres2[] 	= 'LOWER(a.datdescription) LIKE '.$search;
				$wheres2[] 	= 'LOWER(a.meta_keywords) LIKE '.$search;
				$wheres2[] 	= 'LOWER(a.meta_description) LIKE '.$search;
		...
		. ' INNER JOIN #__eventlist_categories AS c ON c.id = a.catsid'
		. ' WHERE ( '.$where.' )'
		. ' AND a.published = 1'
		. ' AND c.published = 1'
	...
	if(in_array('elvenues', $areas)) {
		...
		switch ($phrase) {
			case 'exact':
				$search 		= $db->Quote( '%'.$db->getEscaped( $text, true ).'%', false );
				$wheres2 	= array();
				$wheres2[] 	= 'LOWER(venue) LIKE '.$search;
				$wheres2[] 	= 'LOWER(locdescription) LIKE '.$search;
				$wheres2[] 	= 'LOWER(city) LIKE '.$search;
				$wheres2[] 	= 'LOWER(meta_keywords) LIKE '.$search;
				$wheres2[] 	= 'LOWER(meta_description) LIKE '.$search;
				$where 		= '(' . implode( ') OR (', $wheres2 ) . ')';
				break;
	...
	if(in_array('elcategories', $areas)) {
		...
		switch ($phrase) {
			case 'exact':
				$search 		= $db->Quote( '%'.$db->getEscaped( $text, true ).'%', false );
				$wheres2 	= array();
				$wheres2[] 	= 'LOWER(catname) LIKE '.$search;
				$wheres2[] 	= 'LOWER(catdescription) LIKE '.$search;
				$wheres2[] 	= 'LOWER(meta_keywords) LIKE '.$search;
				$wheres2[] 	= 'LOWER(meta_description) LIKE '.$search;
				$where 		= '(' . implode( ') OR (', $wheres2 ) . ')';
				break;
