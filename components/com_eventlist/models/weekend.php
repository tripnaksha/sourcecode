<?php
/**
 * @version 1.0 $Id: day.php 662 2008-05-09 22:28:53Z schlu $
 * @package Joomla
 ...
 * @since		0.9
 */
class EventListModelWeekend extends JModel
{
	/**
	 * Events data array
	 *
	...
	function _buildQuery()
	{
		...
		$query = 'SELECT a.id, a.dates, a.enddates, a.times, a.endtimes, a.title, a.created, a.locid, a.datdescription,'
				. ' l.name as venue, l.name as city, l.name as state, l.name as url,'
...
//		$where .= ' AND ((\''.$WeekDate[1].'\' BETWEEN (a.dates) AND (IF (a.enddates >= now(), a.enddates, \''.$nulldate.'\')) OR \''.$this->_date.'\' = a.dates) OR (\''.$WeekDate[7].'\' BETWEEN (a.dates) AND (IF (a.enddates >= now(), a.enddates, \''.$nulldate.'\')) OR \''.$this->_date.'\' = a.dates))';
		...
		if ($params->get('filter'))
		{
			...
			if ($filter)
			{
				...
				switch ($filter_type)
				{
					...
					case 'venue' :
						$where .= ' AND LOWER( l.name ) LIKE '.$filter;
						break;
