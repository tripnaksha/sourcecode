...
class EventListModelEditevent extends JModel
{
	...
	function getVenues( )
	{
		...
		if($tsk)
		{
			$query = "SELECT l.id, l.name as venue, CASE WHEN CHAR_LENGTH( m.name ) THEN m.name ELSE 'Guest' END as city, l.createTime as country, l.userId as published"
				.' FROM #__trailList AS l LEFT JOIN #__users AS m'
				.' ON l.userId = m.id'
				.' WHERE l.id = ' . $tsk;
		}
		else
...
//			$where[] = 'LOWER(l.city) LIKE "%'.$filter.'%"';
		...
		$where = $onquery . ' WHERE ' . implode( ' AND ', $where );
