...
<script type="text/javascript">
		...
		function submitbutton( pressbutton ) {
			...
			var arrDate = form.dates.value.split("-");
			var depDate = form.enddates.value.split("-");
			var today = new Date();
			var iniDate = new Date(arrDate[0] + "," + arrDate[1] + "," + arrDate[2]);
			var finDate = new Date(depDate[0] + "," + depDate[1] + "," + depDate[2]);
			title.replace(/\s/g,'');
			...
			if ( title.length==0 ) {
  				...
  				form.enddates.focus();
  				return false;
  			} else if (parseInt(iniDate.valueOf()) < parseInt(today.valueOf())) {
   				alert("<?php echo JText::_( 'DATE OLD', true ); ?>");
   				validator.handleResponse(false,form.dates);
   				form.dates.focus();
   				return false;
  			} else if (parseInt(finDate.valueOf()) < parseInt(iniDate.valueOf())) {
   				alert("<?php echo JText::_( 'DATE END', true ); ?>");
   				validator.handleResponse(false,form.enddates);
   				form.dates.focus();
   				return false;
    ...
    <form enctype="multipart/form-data" id="adminForm" action="<?php echo JRoute::_('index.php') ?>" method="post" class="form-validate">
    	...
    	<fieldset class="el_fldst_details">
          ...
          <div class="el_venue floattext">
              ...
              <input type="text" id="a_name" value="<?php if (JRequest::getVar( 'tsk' )) echo JRequest::getVar( 'name' ); else echo $this->row->venue; ?>" disabled="disabled" />
              <!--input type="text" id="a_name" value="<?php if (JRequest::getVar( 'tsk' )) echo $row->venue; else echo $this->row->venue; ?>" disabled="disabled" /-->
              ...
              <div class='el_buttons floattext'>
                  ...
                  <?php if ( $this->delloclink == 1 && !$this->row->id ) : //show location submission link ?>
              		<button type="button" onclick="window.location='/index.php?option=com_traildisplay';" title="Create a trail from the home page"><?php echo JText::_( 'DELIVER NEW VENUE' ); ?></button>
              		<!--button type="button" onclick="submitbutton('addvenue');"><?php echo JText::_( 'DELIVER NEW VENUE' ); ?></button-->
                	<?php endif; ?>
                  <a class="el_venue_select modal" title="<?php echo JText::_('SELECT'); ?>" href="<?php echo JRoute::_('index.php?view=editevent&layout=choosevenue&tmpl=component&tsk=' . JRequest::getVar( 'tsk' )); ?>" rel="{handler: 'iframe', size: {x: 650, y: 375}}">
                      <span><?php echo JText::_('SELECT')?></span>
                  </a>
                  <input class="inputbox required validate-venue" type="hidden" id="a_id" name="locid" value="<?php if (JRequest::getVar( 'tsk' )) echo JRequest::getVar( 'tsk' ); else echo $this->row->locid; ?>" />
              </div>
          </div>
