...
/*	case "name":
	{
		...
		if ($userid != 0)
		{
		   $part1 = " SELECT a.id, a.name, a.nname, a.nemail, a.routeXML, a.detailXML, a.routeStart, a.mapCenter, \n" .
		   	    " a.zoomLevel, a.userId, a.intro, a.length, DATE_FORMAT(a.createtime,'%d %b %y') as ttime, d.name as uname, upload, encodeurl\n" .
		...
		$part2 = " SELECT b.id, b.name, b.nname, b.nemail, b.routeXML, a.detailXML,  b.routeStart, b.mapCenter, \n" .
			 " b.zoomLevel, b.userId, b.intro, b.length, DATE_FORMAT(b.createtime,'%d %b %y') as ttime, c.name as uname, upload, encodeurl\n" .
...
/*		$query = " SELECT a.id, a.name, a.nname, a.routeXML, a.detailXML,  a.routeStart, a.mapCenter, \n" .
			" a.zoomLevel, a.userId, b.descr, a.length, DATE_FORMAT(a.createtime,'%d %b %y') as ttime, d.name as uname, a.upload, a.encodeurl\n" .
			" FROM jos_trailAddlInfo b, jos_trailList AS a LEFT JOIN jos_users d \n" .
			" ON a.userId = d.id \n" .
			" WHERE a.id = " . $trailid . "\n" .
			" AND b.trail_id = a.id";
*/		$query = " SELECT a.id, a.name, a.nname, a.routeXML, a.detailXML,  a.routeStart, a.mapCenter, \n" .
			" a.zoomLevel, a.userId, CASE WHEN CHAR_LENGTH(b.descr) THEN b.descr ELSE '--No Description--' END as descr, a.length, DATE_FORMAT(a.createtime,'%d %b %y') as ttime, d.name as uname, a.upload, a.encodeurl\n" .
			" FROM jos_trailList AS a LEFT JOIN jos_trailAddlInfo b \n" .
			" ON b.trail_id = a.id \n" .
	...
	case "min":
	{
		...
		$query = " SELECT a.id, a.name, a.nname, a.nemail, a.routeXML, a.detailXML,  a.routeStart, a.mapCenter, \n" .
			" a.zoomLevel, a.userId, b.descr, a.length, DATE_FORMAT(a.createtime,'%d %b %y') as ttime, d.name as uname, upload, encodeurl\n" .
			" FROM jos_trailList AS a, jos_trailAddlInfo b, jos_users d \n" .
			" WHERE a.userId = " . $userid . "\n" .
			" AND b.trail_id = a.id \n";
			" AND a.userId = d.id \n";
		break;
	...
	case "all":
	{
		...
		$query = " SELECT a.id, a.name, a.nname, a.routeXML, a.detailXML,  a.routeStart, a.mapCenter, \n" .
			" a.zoomLevel, a.userId, a.intro as descr, a.length, DATE_FORMAT(a.createtime,'%d %b %y') as ttime, 'uname', upload, encodeurl\n" .
