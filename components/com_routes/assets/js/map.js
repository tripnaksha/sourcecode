...
function drawWayMarkerauto (poly) {
	...
	trailLength = calcDistance();
        var temp = document.getElementById('traillen');
        temp.innerHTML = "Length of Trail - " + trailLength + " km.";
...
function beginRecording() {
	...
	var infoShown = readCookie("infoShown");
	if(infoShown){
	}
	else
	{
...
function setRecordMode() {
	if (srchFlag == 1 && strtFlag != 1)
	   if (confirm ('Reset map?'))
	     map.setCenter(new GLatLng(startLat, startLng), parseInt(startZoom));
...
{
	var response = text.responseText;
	if (response.indexOf("Trail Saved.") != -1)
	{
	   if (disControl)
	   {
...
//	   fadeOut("mesControl","disDiv","50","2000");
	   ...
	   var trailid = response.substring(response.indexOf(".")+1, response.indexOf(":"));
	   var trailname = response.substring(response.indexOf(":")+1);
	   trailname = trailname.replace (/\s+/g, "-");
	   SqueezeBox.fromElement('index.php?option=com_addltrailinfo&tmpl=component&trailid='+trailid+'&trailname='+trailname, {handler: 'iframe', size: {x: 600, y: 500}});
...
//alert("sd "+jsonText.responseText);
