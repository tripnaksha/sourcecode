...
//	<!-- Core Files.  Change the hyperlink references to reflect your site structure.  Note, this must also be updated in the ratings.js file. -->
	...
	$document->addScript('/modules'.DS.'mod_ajaxsearch'.DS."js/jquery-1.3.2.min.js");
...
/*	$query = " SELECT b.id, b.name as tname, b.nname, b.nemail, b.routeStart, b.mapCenter, \n" .
		 "  b.userId, b.length, DATE_FORMAT(b.createtime,'%d %b %y') as ttime, b.intro, c.name as uname, b.routeXML\n" .
		 " FROM #__trailList AS b LEFT JOIN #__users c \n" .
		 " ON b.userId = c.id \n" .
		 " WHERE b.private = 0 \n" .
...
?>
	<tr class="trailtableentry1">
		<td>
		<a class="imga" href="<?php echo JURI::base() . 'index.php?option=com_traildisplay&Itemid=1&tview=' . $row->id . '&trailname=' . str_replace(' ', '-', $row->tname);?>" title="Map for the trail <?php echo $row->tname;?>"><img alt="<?php echo $row->tname;?>" src="http://maps.google.com/maps/api/staticmap?size=80x80&amp;path=weight:3|color:red|enc:<?php echo $polyline[0]; ?>&amp;key=<?php echo $key; ?>&amp;sensor=false&amp;maptype=hybrid"/></a>
		</td>
		<td class="detailcol">
		  <?php echo  "<h3><a href=\"" . JURI::base() . "index.php?option=com_traildisplay&Itemid=1&tview=" . $row->id . '&trailname=' . str_replace(' ', '-', $row->tname) . "\" title=\"Map for the trail " . $row->tname ."\">" . strtoupper($row->tname) . "</a></h3>" . " created on " . $row->ttime . " by " . ($row->uname?"<a href=\"index.php?option=com_comprofiler&task=userProfile&user=".$row->userId."\">".$row->uname."</a>":($row->nname?$row->nname:"Guest"));?>
		  <br />
		  <h4>Description</h4>
		  <?php echo  $row->intro;?>
		</td>
