 ...
 * @since		1.5
 */
class LoginBoxModelRemind extends JModel
{
	/**
	 ...
	 * @var	string
	 */
//	var $_namespace	= 'com_login_box.remind.';
	...
	function remindUsername($email)
	{
		...
		//Added - Ajay - 24/03/09 - check random arithmetic question
		if (!$this->_checkSum()) {
			$this->setError(JText::_('you entered the wrong sum. Please add and enter sum again.'));
			return false;
		}
	...
	function _sendReminderMail($email, $username)
	{
		...
		$uri		= &JFactory::getURI();
		$url		= $uri->toString( array('scheme', 'host', 'port')).JRoute::_('/', false);
